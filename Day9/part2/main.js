...
for(let i = 0; i<puzzleInput.length; i++){
    if(puzzleInput.charAt(i) == "("){
        let quant = puzzleInput.substring(i+1, puzzleInput.indexOf(")", i));
        let neededChars = parseInt(quant.split("x")[0]);
        let multiplier = parseInt(quant.split("x")[1]);
        let startIndex = puzzleInput.indexOf(")", i) + 1;
        for(let j = startIndex + neededChars -1 ; j>i; j--){
            if(puzzleInput.charAt(j) == "("){
                let nestedQuant = puzzleInput.substring(j+1, puzzleInput.indexOf(")", j));
                let nestedNeededChars = parseInt(nestedQuant.split("x")[0]);
                let nestedMultiplier = parseInt(nestedQuant.split("x")[1]);
                let nestedStartIndex = puzzleInput.indexOf(")", j) + 1;
                for(let k = nestedStartIndex; k < nestedStartIndex + nestedNeededChars; k++){
                    if(isAValidChar(puzzleInput, k)){
                        puzzleArr[k] = puzzleArr[k] * nestedMultiplier;
                    }
                }
            }
        }
        for(let j = startIndex; j < startIndex + neededChars; j++){
            if(isAValidChar(puzzleInput, j)){
                puzzleArr[j] = puzzleArr[j] * multiplier
            }
        }
        i += quant.length + neededChars + 1;
    }
}
let total = 0;
